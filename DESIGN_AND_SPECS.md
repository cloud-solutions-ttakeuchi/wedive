...
- **マイレビュー (My Reviews)**: 自身が投稿した全レビュー（承認待ち含む）の一覧表示。ここから編集・削除が可能。
...
| `ENABLE_V2_VERTEX_SEARCH` | Managed RAG 有効化フラグ | `true` の場合、Vertex AI Search を使用してコンシェルジュが回答します。 (旧: `USE_VERTEX_AI_SEARCH`) |
