...
USING (
  ...
  FROM `{PROJECT_ID}.{DATASET_ID}.points_raw_latest`
) s
ON t.id = s.id
WHEN MATCHED AND (t.name != s.name OR t.search_text IS NULL) THEN
  UPDATE SET
    ...
    search_text = CONCAT(s.name, ' ', `{PROJECT_ID}.{DATASET_ID}.fn_to_kana`(s.name), ' ', IFNULL(s.area_name, '')),
...
WHEN NOT MATCHED THEN
  ...
  VALUES (
    ...
    CONCAT(s.name, ' ', `{PROJECT_ID}.{DATASET_ID}.fn_to_kana`(s.name), ' ', IFNULL(s.area_name, '')),
    CURRENT_TIMESTAMP()
  );
