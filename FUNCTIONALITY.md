...
- **検索・ライブフィード**:
  ...
  - **レビュー閲覧権限ロジック**:
    - **承認済み**: すべてのユーザー（ゲスト含む）が閲覧可能。
    - **承認待ち**: 「管理者・モデレーター」または「投稿者本人」のみが閲覧可能。これにより、投稿直後の内容確認と公開前の審査を両立。
...
- 形式: `ENABLE_V{メジャーバージョン}_{機能名}` （例: `ENABLE_V6_REVIEWS`）
- 目的: 実装されたバージョンを明示し、将来的なクリーンアップを容易にします。

### 6.2 主な制御フラグ
- **ENABLE_V2_AI_CONCIERGE**: AI コンシェルジュ機能の表示制御。
- **ENABLE_V6_REVIEWS**: ユーザーレビューシステムの有効化。
