...
- **Local-First アーキテクチャ (v6.3.0+)**:
  - **SQLite 駆動**: 個人ログ (`user.db`) およびマスタ (`master.db`) を SQLite で管理し、通信環境がない場所でも高速に表示・検索が可能。
  - **Firestore 監視の廃止**: 運用コスト削減とバッテリー効率向上のため、`onSnapshot`（常時同期）を全廃。データ取得は「初回起動時」または「将来の手動更新」のみに限定。
  - **オフライン保存**: 圏外で登録・編集した内容はまず SQLite に保存され、ネットワーク復帰時に Firestore へ 1回だけバックアップされる。
