...
1.  **チケット消費トランザクション (AI Concierge Consumption)**
    ...
    *   **Step 3 (Write)**:
        *   有効なチケットが見つかった場合、そのドキュメントを更新 (`status='used'`, `remainingCount=0`)。
...
2.  **ローカル同期フロー (Local Sync Flow)**
    ...
    *   **アクション**: `UserDataService.saveTicket()` 等を使用し、SQLite の `my_ai_concierge_tickets` テーブルにある該当チケットの `remaining_count` を 0 に更新する。
...
4.  **ロールバックとエラーハンドリング戦略 (Rollback & Error Handling)**
    ...
    *   **ローカル同期失敗時 (部分的な失敗)**:
        *   Firestore 更新成功後、SQLite 更新中にエラー（アプリ終了やDBエラー）が発生した場合、一時的に**データ不整合**（Firestore=消費済み, ローカル=未消費）が発生する。
        *   **回復策**: 次回起動時や画面リロード時に実行される `syncTickets()` (Firestore からの最新状態取得) によって、ローカルデータはサーバー側の正解データで上書き補正される。
        *   **UI上の振る舞い**: ユーザーには一時的に「チケットがある」ように見える可能性があるが、消費アクションを実行した時点で Firestore 側のステータスチェックによりエラーとなるため、不正利用のリスクはない。エラー発生時に最新データを再取得して UI を更新することで解決する。
