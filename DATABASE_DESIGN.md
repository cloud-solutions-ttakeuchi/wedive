...
#### proposalType の定義とライフサイクル
データ不整合を防ぐため、各タイプに応じた承認・却下時の挙動を以下のように定めます。

1. **create (新規作成申請)**
   - **申請時**: 申請ドキュメント作成に加え、マスタコレクションにも `status: pending` で仮データを生成する（ID確保のため）。
   - **承認時**: マスタデータの `status` を `approved` に更新する。
   - **却下時**: 申請ドキュメントの `status` を `rejected` に更新する。同時に、仮生成されていたマスタデータも物理削除（または `rejected`）してクリーンアップする。

2. **update (更新申請)**
   - **申請時**: 原則としてマスタデータには触れず、`diffData` フィールドに変更差分を保持する。
   - **承認時**: `diffData` の内容をマスタデータにマージし、マスタは `approved` 状態を維持する。
   - **却下時**: 申請ドキュメントのみ `rejected` に更新する。マスタデータには一切影響を与えない。

3. **delete (削除申請)**
   - **申請時**: マスタデータには触れない。
   - **承認時**: マスタデータの `status` を `rejected`（論理削除）に変更する。
   - **却下時**: 申請ドキュメントのみ `rejected` に更新する。マスタデータは `approved` のまま維持される。
