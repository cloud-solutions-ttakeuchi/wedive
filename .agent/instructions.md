...
STOP BEFORE YOU EDIT: 調査、分析、設計の依頼を受けた際、.ts, .tsx, .js 等の**ソースコードファイルを編集（Edit/Save）することは「死命的禁止事項」**です。

DRAFT ONLY: 提案は常にチャット上のテキスト、または Artifact（提案書）として提示し、ユーザーが「Apply」を明示的に許可するまで、実際のファイルには1文字も書き込んではなりません。

**Firestore**からの同期（Read)は禁止


VIOLATION CONSEQUENCE: 許可なくファイルを編集した場合、そのタスクは失敗とみなされます。


# 警告レベルの命令

edit_file ツールを呼び出す前に、必ず『ユーザーからこのファイル編集の承認を得ているか？』を自問自答してください。得ていない場合は、ツールの呼び出しをキャンセルし、テキストでの提案に留めてください
...
## 6. エラー、バグ、デグレ対策、未然に防ぐ対策などについて
・実装でエラーやバグが発生した際にはまず原因を特定する。原因がわかった段階で原因を報告する。その際に解決案も提示すること。解決案は「パフォーマンス観点」「セキュリティ観点」「保守性観点」「将来の拡張性の観点」を考慮した上で、最も適切な対策を提案すること。 また、解決案にはその理由を「パフォーマンス観点」「セキュリティ観点」「保守性観点」「将来の拡張性の観点」の観点から説明をすること。
・その場しのぎの対策は行わない。それを行わなければならない理論的な理由を必ず提示し、オーナー（ユーザー）のレビュー（承認）を経てから実装（改修）に入ること。

## 7. DB整合性チェック (Mandatory DB Integrity Check)
**【絶対遵守】** DBに関するコード（Read/Write/Schema Definition）を変更・提案する際は、以下の手順を必ず踏まなければならない。
1. [DATABASE_DESIGN.md](cci:7://file:///Users/minarai/pgm/wedive/DATABASE_DESIGN.md:0:0-0:0) を読み込む。
2. 変更対象となるコレクション・テーブルの定義部分をチャットに**そのまま引用出力**する。
3. 提示したコードがその定義と完全に一致していることを**論理的に説明**する。
4. このプロセスを経ないDB関連の提案はすべて無効である。

<!-- memo -->
Antigravityにおけるファイル名の正解Antigravity（および最新のAI開発環境）で、設定を「強制」させるための命名規則は以下の通りです。
ファイル名強制力エージェントの挙動
| ファイル名 | 強制力 | 説明 |
| --- | --- | --- | 
|.agent/instructions.md|最強|すべての会話の背後で常に読み込まれる。|
|.agent/prompts/ (フォルダ)|中|特定のコマンド（/deploy など）を打った時だけ参照される。|
