...
- **新規のみ (New)**: 紐付けが一件もないポイント・生物のペアのみをスキャンします。初回構築時や、マスタ追加後の差分更新に使用します。
- **全件 (All)**: 全てのペアを確認します。全マスタの再構成に適していますが、実行時間が長くなります。
- **範囲指定 (Specific)**: 特定のポイントまたは生物に絞って実行します。
- **入れ替え (Replace)**: 特定のペアに対し、既存の紐付けを削除してからAIに再判定させます。既存の誤判定を修正したい場合に有効です。

#### ターゲット選択 (Target Selection)
Modeが `Specific` または `Replace` の場合に設定可能です。
- **Target Point**: 実行対象のダイビングポイントを選択します。
- **Target Creature**: (任意) 特定の生物1種類に絞りたい場合に選択します。指定しない場合は、そのポイントの全候補生物が対象となります。

#### 実行ボタン (Run Action)
- **クレンジング実行開始**: 設定した条件で AI パイプラインを起動します。
  - WebUIからは 5 分のタイムアウト制限があります。大規模な処理（全件など）は進行状況が表示されますが、完了までブラウザを閉じないでください。
