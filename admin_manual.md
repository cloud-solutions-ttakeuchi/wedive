...
#### A. 2 段階の判定プロセス (The 2-Stage Pipeline)
...
1.  **Stage 1: 物理的生存・分布判定 (Batch Potential Check + Context Caching)**
    ...
    *   **技術的最適化**: 「生物辞書」を **Gemini Context Cache** に展開することで、1000 回、2000 回といった大量の判定でもトークン消費を最小限（約 1/4 以下）に抑えています。
2.  **Stage 2: 事実確認・目撃実績の精査 (Grounding with Google Search)**
    *   **入力**: Stage 1 で「生息可能」と判定されたペア。
    *   **処理**: **Google Search Tool** を動員し、Web 上から実際のログ情報やショップのブログ、目撃記録をリアルタイムに検索します。
    *   **出力**: 「実在（actual_existence）」の有無、検索で見つかった「証拠（evidence）」の要約、推定される「希少度（rarity）」を確定させます。

#### B. 内部判定スコアリング

*   **信頼度 (Confidence)** の算出式：
    *   `Stage 1 の確信度(0.1〜0.9) + (実在確認が成功した場合に加算) / 補正係数`
