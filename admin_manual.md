...
### Export JSON (バックアップ)
画面上部の「Export JSON」ボタン（緑色）をクリックすると、現在のFirestore上の全場所データをJSONファイルとしてダウンロードできます。
- **形式**: `locations_seed.json` と互換性のある形式です。
- **用途**: データのバックアップ、または開発環境のシードファイル(`src/data/locations_seed.json`)の更新に使用します。
