...
AIを活用して、ダイビングポイントと生物の紐付け（生息・出現情報）を自動検証・生成・管理する高度なシステムです。

### 5.1 基本コンセプト
- **生物学的検証**: AIが生態、水深制限、地理的分布に基づき、そのポイントにその生物が「生息可能か」を一次判定します。
- **事実確認 (Grounding)**: 一次判定で可能性が高いものに対し、Google検索を実行して「実際の目撃例」や「ポイント特有の出現情報」を二次判定します。
- **管理者レビュー**: AIの判断は必ず `pending`（保留）状態で保存され、人間が最終確認を行うワークフローです。

### 5.2 操作モード
Dashboardタブから以下の実行モードを選択できます。
- **新規 (New)**: まだ紐付けが存在しないペアのみを対象に実行します。
- **リセット (All)**: 既存の全紐付けを無視して、一から再計算します。
- **範囲指定 (Specific)**: 特定のポイント、または特定の生物を指定してピンポイントで実行します。
- **入れ替え (Replace)**: 特定のペアに対し、既存データを削除してAIの最新結果で上書きします。

### 5.3 Review Engine (レビュー機能)
AIによって生成された提案を確定させるためのインターフェースです。
- **Point-based View**: 1つのポイントに対して、複数の生物候補をリストアップ。
- **Creature-based View**: 1つの生物に対して、目撃される可能性があるポイントをリストアップ。
- **信頼度スコア**: 0.0〜1.0で表示。0.8以上は精度が非常に高く、即座に承認可能です。
- **Reasoning (根拠)**: AIが「なぜそう判断したか」「証拠となるURL/情報」を表示します。
- **一括承認/却下**: 複数項目を選択して、一括で「承認（本番反映）」または「却下（データ削除）」が可能です。

### 5.4 運用上の注意
- **コスト最適化**: システムは `Context Caching` を使用し、API費用を 75% 削減する設計になっていますが、全件リセット等は時間とリソースを要します。
- **バックグラウンド実行**: WebUIからの実行は5分でタイムアウトするため、大規模な処理は別途 Cloud Run Jobs 等で自動実行されるバッチ処理に委ねる運用を推奨します。
- **データ保護**: 処理前には必ずシードデータのバックアップ（`.bak`）が作成されます。
